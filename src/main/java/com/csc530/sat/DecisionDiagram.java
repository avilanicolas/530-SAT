...
public interface DecisionDiagram {
    ...
    * simplified if the value of any variable is known. For example, D.assume(z,
    * true) should constrain the diagram to be equivalent to true D.assume(x,
    * false) should constrain the diagram to be equivalent to z.
    *
   ...
   /**
    * Finds all the possible cases that satisfy this boolean equation. Note that
    * not all variables my be mapped in this representation, for mappings that
    * contain less than the number or variables the variables that are not
