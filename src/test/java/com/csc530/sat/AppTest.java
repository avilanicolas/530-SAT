...
public class AppTest extends TestCase {
    ...
    public void demo() {
        ...
        assertEquals(DecisionDiagramLeaf.UNSATISFIABLE, complex.assume(varA, false));
        ...
        assertTrue(complex.assume(varC, false).satisfies(
                // C is no longer needed in this assignment
                assignment(varA, true, varB, true, varD, false)));

        // You could also reduce a decision diagram to just your unknown
        // variables
        assertEquals(c, complex.assume(varA, true).assume(varB, true).assume(varD, true));
        // Or reduce it to trivially satisfiable or unsatisfiable
        assertEquals(DecisionDiagramLeaf.SATISFIABLE, complex.assume(varA, true)
                .assume(varB, true).assume(varC, true).assume(varD, true));
    ...
    private Map<SATVariable, Boolean> assignment(final Object... assignments) {
        ...
        for (int i = 0; i < assignments.length;) {
            map.put((SATVariable) assignments[i++], (Boolean) assignments[i++]);
        }
